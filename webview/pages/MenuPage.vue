  ...
  <div class="size-full flex items-center justify-center">
    <div class="flex flex-col items-center gap-y-8">
      ...
      <div id="buttons" class="grid grid-cols-2 max-w-2xs w-full gap-4">
        ...
        <button
          type="button"
          class="size-full bg-slate-200 border-2 border-slate-300 p-4 aspect-square rounded-xl flex flex-col gap-y-2 items-center justify-center text-slate-800 not-disabled:cursor-pointer hover:scale-103 transition-transform active:scale-97"
          @click="appStore.navigateTo(Page.CREATE_PUZZLE_SELECT_TOPIC)"
        >
          <i-noto-plus class="text-5xl" />
          <span class="text-xl font-medium text-slate-900">Create</span>
        </button>

        <button
          type="button"
          class="size-full bg-slate-200 border-2 border-slate-300 p-2 rounded-xl flex gap-x-2 items-center justify-center text-slate-800 not-disabled:cursor-pointer hover:scale-103 transition-transform col-span-2 active:scale-97"
          @click="appStore.navigateTo(Page.LEADERBOARD)"
        >
          <i-noto-trophy class="text-2xl" />
          <span class="text-xl font-medium text-slate-900">Leaderboard</span>
        </button>
        ...
        >
          ...
          <span class="text-xl font-medium text-slate-900">What is this?</span>
        </button>
      </div>
    </div>
  </div>
